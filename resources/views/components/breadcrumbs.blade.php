...
<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        <li class="inline-flex items-center">
            <a
                href="{{ route('teams.documents.index', $folder->team) }}"
        ...
        @foreach($folder->getPath() as $pathFolder)
            <li>
                <div class="flex items-center">
                    ...
                    @if($pathFolder->id === $folder->id)
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $pathFolder->name }}</span>
                    @else
                        <a
                            href="{{ route('teams.documents.folder', [$folder->team, $pathFolder->slug]) }}"
